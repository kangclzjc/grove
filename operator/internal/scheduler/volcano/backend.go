...
// Backend implements scheduler.Backend for the Volcano scheduler.
...
//
// A PodGroup is skipped if MinReplicas == 0 or it has no PodReferences.
// NetworkTopology is set only when the PodGroup has a topology constraint and keyToTier maps the key.
